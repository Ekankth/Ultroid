...
_buttons = {
    "otvars": {
        "text": "Other Variables to set for @:",
        "buttons": [
            [
    ...
    "oofdm": {
        "text": "About [Dual Mode](https://t.me/AnekaShastraHasta",
        "buttons": [
            [
    ...
    if not match:
        try:
            ...
            return await okk.edit(
                f"**• AnekaShastrahasta Userbot •**{cli}", file=img, buttons=button
            )
        except Exception as er:
            LOGS.exception(er)
...
async def _(e):
    ...
    async with asst.conversation(e.sender_id) as conv:
        ...
        if GDrive._create_token_file(code=code.text):
            await conv.send_message(
                "`Success!\nYou are all set to use Google Drive with AnekaShastraHasta Userbot.`",
...
async def _(e):
    if not e.is_private:
        return
    await e.edit(
        "Click Authorise and send the code.\n\nYou can use your own CLIENT ID and SECRET by [this](https://t.me/AnekaShastraHasta)",
        buttons=[
            [
